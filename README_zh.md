# local ASR API — 本地语音转文字 API

local ASR API 是一个本地运行的、兼容 OpenAI 格式的语音转文字（ASR）API 服务。支持多种后端模型，可与 Spokenly 等前端语音输入应用配合使用，无需将音频上传到云端。

---

## 1. 如何使用 local ASR API 与当前支持的模型
...
local ASR API 实现与 **OpenAI Whisper API** 兼容的接口，因此可与支持「OpenAI 兼容 API」的语音输入应用配合使用，例如 **Spokenly**。
...
3. 填写：
   - **URL**：`http://127.0.0.1:8000`（确保与 local ASR API 启动的地址一致）
   - **Model**：与当前使用的模型一致，例如 `mlx-community/Qwen3-ASR-1.7B-8bit` 或 `iic/SenseVoiceSmall`
   - **API Key**：本地服务可不校验，填写任意值（如 `anything`）即可
4. 点击 **Test & Save** 测试并保存。

### 数据流说明

- 语音由 **Spokenly** 采集并发送到本机 **local ASR API**。
- local ASR API 使用本地模型进行转录，**音频不离开本机**，适合注重隐私的场景。
...
以下为 local ASR API 服务与 Spokenly 配合时的整体架构（Mermaid 图可在支持 Mermaid 的 Markdown 预览中查看）。
...
flowchart TB
    ...
    subgraph API["local ASR API 本地服务 (http://127.0.0.1:8000)"]
        direction TB
        FastAPI["FastAPI 服务"]
        FastAPI --> Router["路由层"]
    ...
    Spokenly -->|"上传音频 (multipart)"| FastAPI
    FastAPI -->|"JSON: { text }"| Spokenly
```
...
sequenceDiagram
    ...
    participant R as local ASR API (FastAPI)
