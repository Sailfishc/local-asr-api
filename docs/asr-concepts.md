...
框架解决的是"谁来跑计算"，格式解决的是"权重怎么存到磁盘"。两者是独立的维度，同一套权重数值可以转换成不同格式：

```
原始 PyTorch 权重（.safetensors）
    │
    ├─ mlx_lm.convert   ──→ MLX 权重（.safetensors，内部布局不同）
    ├─ llama.cpp convert ──→ GGUF（供 Ollama / llama.cpp 使用）
    └─ onnxruntime export──→ ONNX（供跨平台 / 边缘设备部署）
```

| 格式 | 创建者 | 存什么 | 谁用 | 特点 |
|------|--------|--------|------|------|
| `.bin` / `.pkl` | PyTorch | 权重 | PyTorch | 基于 pickle，加载时可执行任意代码，有安全风险 |
| `.safetensors` | HuggingFace | 权重 | PyTorch、MLX、JAX | 安全，支持内存映射，加载快；PyTorch 和 MLX 都用这个格式，但内部张量布局不同 |
| GGML | Georgi Gerganov | 权重 + 结构 | llama.cpp 旧版 | 已废弃，被 GGUF 取代 |
| GGUF | Georgi Gerganov | 权重 + 结构 + 元数据 | llama.cpp、Ollama、LM Studio | 自描述（KV 元数据头），兼容性强，支持 4-bit / 8-bit 量化 |
| ONNX | 微软 / Meta | 计算图 + 权重 | 跨框架部署 | 一次导出，到处运行；常用于移动端、嵌入式和 Windows 推理 |

**GGUF 的文件结构**（自描述是它的核心优势）：

```
┌──────────────┐
│ Magic + 版本 │  固定头，标识这是 GGUF 文件
├──────────────┤
│  KV 元数据区 │  架构类型、层数、注意力头数、词表大小……
├──────────────┤
│  张量索引区  │  每个权重张量的名字、形状、偏移量
├──────────────┤
│  张量数据区  │  实际权重（可量化为 4-bit / 8-bit 等）
└──────────────┘
```

旧版 GGML 没有元数据头，模型结构稍有变化就和旧版工具不兼容。GGUF 加了 KV 头之后实现了向前/向后兼容，这才使 Ollama 这类工具成为可能。
...
Transformer 是现代深度学习的基础架构，几乎所有当前的主流 ASR 模型（Whisper、SenseVoice、Qwen3-ASR、Fun-ASR-Nano）都建立在它之上，理解它是理解后续所有架构的前提。

核心机制是**注意力（Attention）**：让模型处理序列中任意一个位置时，能同时"看到"序列中所有其他位置，并根据相关性动态加权。
...
**对比 RNN / LSTM（Transformer 之前的主流）**：

| | RNN / LSTM | Transformer |
|--|------------|-------------|
| 处理方式 | 逐步处理，前一步结果传给后一步 | 所有位置并行计算 |
| 长距离依赖 | 随序列变长而衰减（"记忆力变差"） | 任意两个位置直接交互，无衰减 |
| 训练速度 | 慢（无法并行） | 快（GPU 高度并行） |

**在 ASR 中的角色**：
- **Encoder** 用 Transformer 层提取音频帧之间的关系（什么帧和什么帧语义相关）
- **Decoder** 用 Transformer 层生成文字，同时关注已生成的字符和 Encoder 的输出
- LLM 本质上也是 Transformer，只是规模更大、预训练数据更丰富
...
本地运行 LLM 的一站式工具，底层基于 llama.cpp，使用 **GGUF** 格式。提供 REST API（与 OpenAI 兼容）和命令行模型管理（`ollama pull`、`ollama run`）。

```bash
ollama pull llama3.2        # 下载模型（GGUF 格式）
ollama run llama3.2         # 交互式运行
curl http://localhost:11434/api/generate ...  # REST API
```

**与 RAPL 的关系**：两者定位相似（本地运行、OpenAI 兼容 API），但领域不同——RAPL 专注 ASR（音频 → 文字），Ollama 专注 LLM 文本生成。如果未来 RAPL 需要通过 prompt 注入实现热词，可以把 Ollama 作为 LLM Decoder 的后端。

**与 mlx-audio 的区别**：两者都是"本地推理"，但 Ollama 基于 GGUF / llama.cpp，跨平台（Linux / Windows / Mac 均支持）；mlx-audio 基于 MLX，仅限 Apple Silicon，但在 Mac 上利用统一内存架构更充分。

### vLLM

UC Berkeley 开源的高吞吐量 LLM 推理引擎，设计目标是在**服务器端**（Linux + NVIDIA GPU）最大化并发请求处理能力。

核心创新是 **PagedAttention**：借鉴操作系统虚拟内存的分页思想，把 KV Cache（注意力键值缓存）切成固定大小的块动态管理，大幅减少显存碎片，支持更多并发用户同时推理。

```
KV Cache 传统做法：为每个请求预分配连续显存块 → 碎片严重，并发数受限
PagedAttention：按需分配非连续的显存"页" → 显存利用率提升，并发数大幅增加
```

**与本项目的关系**：vLLM 面向多用户并发的服务端场景（通常跑在 A100 / H100 上），与 RAPL 的单用户本地推理定位完全不同。但如果将来 RAPL 演进为支持多并发的云端 ASR 服务，LLM Decoder 部分就是 vLLM 的适用场景。

### ONNX Runtime

微软主导的跨框架推理引擎，配合 ONNX 格式使用。一个模型用任意框架（PyTorch、TensorFlow 等）训练后，导出为 ONNX，即可在 ONNX Runtime 上运行——不依赖原始训练框架。

**主要使用场景**：
- **移动端 / 嵌入式**：iOS、Android、树莓派等资源受限设备
- **Windows 推理**：DirectML 后端利用 DirectX 12 加速
- **跨平台服务端**：统一不同框架训练的模型的部署方式

**与 GGUF / llama.cpp 的区别**：ONNX 保存的是**计算图**（每一步算什么，怎么算），通用性更强但体积较大；GGUF 只保存**权重 + 元数据**，专为 LLM 推理优化，体积更小，量化支持更好。
...
| 第 1 层 | .safetensors | 两个后端的模型权重文件格式 |
| 第 1 层 | GGUF | 非 RAPL 路径；Ollama 等工具使用 |
| 第 1 层 | ONNX | 非 RAPL 路径；跨平台 / 边缘设备部署使用 |
| 第 2 层 | Transformer | 所有 ASR 模型的基础架构（Encoder / Decoder 均基于它） |
...
| 第 4 层 | Ollama | 非 RAPL 路径；本地 LLM 文本生成；潜在 LLM Decoder 后端 |
| 第 4 层 | vLLM | 非 RAPL 路径；服务端高并发 LLM 推理；RAPL 若演进为云服务时的参考方向 |
| 第 4 层 | ONNX Runtime | 非 RAPL 路径；跨框架统一推理引擎 |
